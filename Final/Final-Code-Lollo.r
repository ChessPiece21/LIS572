...
top_heroes_plot <- ggplot(top_heroes) +
  geom_col(aes(x = Pick_Rate, 
               y = reorder(Hero, +Pick_Rate), 
               fill = Role,
...
heroes_by_role <- ow2_df %>% group_by(Role) %>% summarize(Count = n(), Avg_Pick_Rate = mean(Pick_Rate))

# Create interactive plot of Hero pick rate by role.
hero_role_plot <- ggplot(heroes_by_role) +
  geom_col(aes(x = Avg_Pick_Rate, 
               y = reorder(Role, +Avg_Pick_Rate), 
               fill = Role,
               text = paste0("Hero Role: ", Role,
                             "\nNumber of Heroes: ", Count,
                             "\nAverage Play Rate: ", Avg_Pick_Rate,"%"))) + 
...
gender_plot <- ggplot(heroes_by_gender) +
  geom_col(aes(x = Avg_Pick_Rate, 
               y = reorder(Gender, +Avg_Pick_Rate), 
               fill = Gender,
               text = paste0("Hero Gender: ", Gender,
                             "\nAverage Play Rate: ", Avg_Pick_Rate,"%"))) +
